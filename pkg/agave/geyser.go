...
func (g *GeyserPlugin) ToConfigString() (string, error) {
	if g.YellowstoneGrpc != nil {
		return g.YellowstoneGrpc.ToConfigString()
	} else {
...
type YellowstoneGrpc struct {
	...
	YellowstoneGrpcConfigUnstructured *string                `pulumi:"YellowstoneGrpcConfigUnstructured,optional"`
	Version                           string                 `pulumi:"version"`
}

func (y *YellowstoneGrpc) ToConfigString() (string, error) {
	tempMap := make(map[string]any)
	if y.YellowstoneGrpcConfig != nil {
		// Marshal the original config to get its JSON representation
		origBytes, err := json.Marshal(y.YellowstoneGrpcConfig)
		if err != nil {
			return "", err
		}
		...
		if err := json.Unmarshal([]byte(*y.YellowstoneGrpcConfigUnstructured), &tempMap); err != nil {
			return "", err
		}
	}
	// Add the libpath field
	tempMap["libpath"] = y.libPath()
...
func (y *YellowstoneGrpc) libPath() string {
	return "/usr/lib/libyellowstone_grpc_geyser.so"
}
